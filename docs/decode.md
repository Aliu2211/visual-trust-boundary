...
1. **The decoder is part of the trust boundary.** In default mode zbar can rewrite a QR payload's non-ASCII bytes by encoding inference before the boundary layer sees them, so validation runs on what zbar chose to output, not on what the code carries. Whether it does depends on the content: accented, CJK and invalid-UTF-8 samples were rewritten (E-008, E-020), while fullwidth digits came back exact in default mode (`mal-unicode-001`, E-020).
